"""Ship + run postproc grid search on fteam11."""
import os, paramiko
...
sftp=t11.open_sftp()
for fname in ("postproc_search_v3_remote.py",):
    local=LOCAL/fname
...
print("\n=== postproc grid search ===")
run(f"cd {REMOTE} && python3 -u scripts/postproc_search_v3_remote.py 2>&1 | tee logs/postproc_v3.log",
    t=1800)
t11.close(); j.close()
